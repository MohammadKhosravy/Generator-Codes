...
void  Select::generate_hard_C_and_c(int _n, int _p, int _N, int _R)
{
	n = _n;
	p = _p;
	N = _N;
...
double Select::solve_master_C()
{
	...
	for (int j=0; j<K; ++j)
	{
        ...
        for (int k=0; k<n; ++k)
            betaval -= x[j][k];
        con += betaval*cplexbeta[j];

        for (int k=0; k<n; ++k)
            con += (x[j][k] - 1)*cplexgamma[j][k];
    ...
    for (int j=0; j<K; ++j)
        for (int k=0; k<n; ++k)
        {
            ...
            model.add(cplexbeta[j] <= cplexgamma[j][k] + con);
        }
...
Solution Select::solve_ip(double tlim)
{
	...
	model.add(IloMinimize(env, obj + cplexz));
